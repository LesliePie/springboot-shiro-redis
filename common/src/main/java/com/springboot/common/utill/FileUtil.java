...
public class FileUtil {
    ...
    */ 
    ...
    public static int getFileType(String originalFilename) {
        ...
        if(Arrays.asList(Constant.FilePostFix.EXCEL).contains(postFix)){
            return Constant.FileType.FILE_EXCEL;
        }
        if(Arrays.asList(Constant.FilePostFix.PPT).contains(postFix)){
            return Constant.FileType.FILE_PPT;
        }
        if(Arrays.asList(Constant.FilePostFix.PDF).contains(postFix)){
            return Constant.FileType.FILE_PDF;
        }
        return Constant.FileType.FILE_IMG;
    }
    ...
    public static String readFileContent(File file, String fullFilePath) throws IOException {
        String returnStr = "";
        if (file==null&& StringUtils.isBlank(fullFilePath)) {
            return "";
        }
        if (file==null) {
    ...
    public static boolean writeToFile(String content, File file, String fullFilePath) throws IOException {
        if ((file==null && StringUtils.isBlank(fullFilePath)) || StringUtils.isBlank(content)) {
            return false;
        }
        if (file==null) {
            file = new File(fullFilePath);
