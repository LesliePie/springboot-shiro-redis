...
import com.springboot.common.utill.ComUtil;
        ...
        for (String aClassName:Constant.METHOD_URL_SET) {
            ...
            for (Method method:methods) {
                if(method.isAnnotationPresent(Pass.class)){
                    ...
                    }else if (!ComUtil.isEmpty(method.getAnnotation(PatchMapping.class))){
                        methodUrl=method.getAnnotation(PatchMapping.class).value();
                        if (ComUtil.isEmpty(methodUrl)){
                            methodUrl=method.getAnnotation(PatchMapping.class).path();
                        }
                        baseUrl=getRequestUrl(classUrl,methodUrl,sb,"PATCH");
                    } else {
